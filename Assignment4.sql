...
DROP TABLE employee_logger CASCADE CONSTRAINTS;
DROP TABLE dependent_logger CASCADE CONSTRAINTS;
DROP SEQUENCE region_seq;
DROP SEQUENCE region_seq_temp;
DROP TRIGGER employee_trigger;
DROP TRIGGER dependent_trigger;
...
/
CREATE TABLE dependent_logger (
	dependent_name VARCHAR2 (20) DEFAULT NULL,
    creation_date DATE
);
/
...
BEGIN
   ...
   INSERT INTO employee_logger(employee_name,creation_date) VALUES(:new.first_name, SYSDATE);
END;
/
...
BEGIN
   -- insert a row into the audit table   
   INSERT INTO dependent_logger(dependent_name,creation_date)
   VALUES(:new.first_name, SYSDATE);
END;
/

-- sequence 1
CREATE SEQUENCE region_seq_temp
    START WITH 500
    INCREMENT BY 1;


/*Data for the table regions*/
INSERT INTO regions(region_id,region_name) VALUES (region_seq_temp.NEXTVAL,'Europe');
INSERT INTO regions(region_id,region_name) VALUES (region_seq_temp.NEXTVAL,'Americas');
INSERT INTO regions(region_id,region_name) VALUES (region_seq_temp.NEXTVAL,'Asia');
INSERT INTO regions(region_id,region_name) VALUES (region_seq_temp.NEXTVAL,'Middle East and Africa');
...
UPDATE regions SET region_id = region_seq.NEXTVAL;
...
-- global variable    
    ...
    FUNCTION total_country_by_region(rg_id countries.region_id%TYPE) RETURN NUMBER;
    -- total employee who is having salary greater than equal to x
    FUNCTION total_employee_greater_salary(slr employees.salary%TYPE) RETURN NUMBER;
    -- get dependent based on dependent id
    PROCEDURE print_dependent( d_id dependents.dependent_id%TYPE );
    -- get employee based on employee id
    PROCEDURE print_employee( e_id employees.employee_id%TYPE );
...
total_employee NUMBER := 0;
    ...
    FUNCTION total_country_by_region(rg_id countries.region_id%TYPE) 
        RETURN NUMBER
        IS
        BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
            RETURN 0;
    END total_country_by_region;
    
    FUNCTION total_employee_greater_salary(slr employees.salary%TYPE) 
        RETURN NUMBER
        IS 
        BEGIN
    ...
    -- return the total employees
        ...
        EXCEPTION
            WHEN OTHERS THEN
            RETURN 0;
    END total_employee_greater_salary;
...
EXEC employee_package.print_employee(105);
EXEC employee_package.print_dependent(2);
--select total_country_by_region(2) from countries;
--select total_employee_greater_salary(5000) from employees;
